...
class ClientesController extends Controller
{
    ...
    public function actionAdmin()
    {
        ...
        $clientes = Clientes::model()->findAll(array('condition'=>'t.estado != 2', 'order'=>'t.id_clientes DESC'));
        ...
        if(isset($_POST['Clientes']))
        {
            ...
            if($model->validate(null, false)){
                if(($_FILES['logo']['size'] > 0)){
                    ...
                    $uploadLogo = MyMethods::uploadImage($_FILES['logo'], 450, 'clientes/');
                    ...
                    if(!$uploadLogo['status']){
                        ...
                        MyMethods::resizeImage($server.'clientes/', $model->imagen, 450, 330);
                    }
                }
                else{
        ...
        if(isset($_POST['Clientes']))
        {
            ...
            if($model->validate(null, false)){
                if(($_FILES['logo']['size'] > 0)){
                    ...
                    $uploadLogo = MyMethods::uploadImage($_FILES['logo'], 250, 'clientes/');
     ...
     */
    public function actionDelete_cliente($id)
    {
        $entry = $this->loadModel($id);

        $entry->estado = 2;
        if($entry->save()){
            @unlink($_SERVER['DOCUMENT_ROOT'].Yii::app()->request->baseUrl."/images/clientes/".$entry->imagen);
            @unlink($_SERVER['DOCUMENT_ROOT'].Yii::app()->request->baseUrl."/images/clientes/450x330/".$entry->imagen);
        }
    ...
    {
        $model=Clientes::model()->findByAttributes(array('id_clientes'=>$id), array('condition'=>'t.estado != 2'));
        if($model===null)
            throw new CHttpException(404,'The requested page does not exist.');
        return $model;
    }
