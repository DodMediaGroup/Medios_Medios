...
class MediosController extends Controller
{
    ...
    public function actionAdmin()
    {
        ...
        $medios = Medios::model()->findAll(array('condition'=>'t.estado != 2', 'order'=>'t.id DESC'));
        ...
        if(isset($_POST['Medios']))
        {
            ...
            if($model->validate(null, false)){
                if(($_FILES['logo']['size'] > 0)){
                    ...
                    $uploadLogo = MyMethods::uploadImage($_FILES['logo'], 400, 'medios/');
     ...
     */
    public function actionDelete_medio($id)
    {
        $entry = $this->loadModel($id);
    ...
    {
        $model=Medios::model()->findByAttributes(array('id'=>$id), array('condition'=>'t.estado != 2'));
        if($model===null)
            throw new CHttpException(404,'The requested page does not exist.');
        return $model;
    }
